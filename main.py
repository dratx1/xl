...
from app.client.engsel import get_balance, get_quota
from app.service.auth import AuthInstance
from app.menus.bookmark import show_bookmark_menu
from app.menus.account import show_account_menu
from app.menus.package import fetch_my_packages, get_packages_by_family, show_package_details
from app.menus.hot import show_hot_menu, show_hot_menu2
from app.menus.anu import show_anu_menu, show_anu_menu2
...
def show_main_menu(number, balance, balance_expired_at, display_quota=None):
    ...
    show_banner()
    expired_at_dt = (
        datetime.fromtimestamp(balance_expired_at).strftime("%Y-%m-%d %H:%M:%S")
        if isinstance(balance_expired_at, (int, float))
        else str(balance_expired_at)
    )
    info = Table.grid(padding=(0, 1))
    info.add_column(justify="right", style=_c("text_sub"))
    info.add_column(style=_c("text_body"))
    info.add_row("          📱", f"[{_c('text_value')}]{number}[/]")
    info.add_row("          💰", f"[{_c('text_money')}]Rp {balance:,}[/]")
    if display_quota:
        info.add_row("          🧮", f"[{_c('text_date')}]{display_quota}[/]")
    info.add_row("          ⏳", f"[{_c('text_date')}]{expired_at_dt}[/]")
    ...
    menu.add_row("[bold]6[/]", "🛒 Beli Paket Lainnya-2 ")
    menu.add_row("[bold]7[/]", "🏠 Input Family Code")
    menu.add_row("[bold]8[/]", "🏢 Input Family Code (Enterprise)")
    menu.add_row("[bold]9[/]", "🏡 Get family (PRE_TO_PRIOH)")
    menu.add_row("[bold]10[/]", "🏬 Get family (PRE_TO_PRIOH + Enterprise)")
    menu.add_row("[bold]00[/]", "⭐ Bookmark Paket")
    menu.add_row("[bold]69[/]", f"[{_c('text_sub')}]🎨 Ganti Gaya[/]")
    menu.add_row("[bold]88[/]", f"[{_c('text_ok')}]👑 Paket Sulthan[/]")
    ...
    while True:
        ...
        if active_user:
            ...
            try:
                ...
                display_quota = None
                pesan_error(f"Gagal mengambil data kuota: {e}")
            ...
            if choice == "1":
                ...
                try:
                    ...
                    pesan_error(f"Gagal menampilkan paket lainnya-2: {e}")
            elif choice == "7":
