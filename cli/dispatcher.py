...
def dispatch(choice: str, ctx: dict):
    match choice:
        case "1":
            ...
            if selected:
                ...
                console.print("[bold red]❌ Gagal memilih user.[/bold red]")
        ...
        case "5":
            code = Prompt.ask("📮 Masukkan option code (atau '99' untuk batal)")
            if code != "99":
                show_package_details(ctx["api_key"], ctx["tokens"], code, False)

        case "6":
            code = Prompt.ask("📁 Masukkan family code (atau '99' untuk batal)")
            if code != "99":
        ...
        case "7":
            ...
            pause_each = Prompt.ask("Pause tiap sukses? (y/n)", default="n").lower() == "y"
        ...
        case "10":
            show_circle_info(ctx["api_key"], ctx["tokens"])
        ...
        case "12":
            ...
            show_family_list_menu(ctx["subscription_type"], is_enterprise)

        case "13":
            is_enterprise = Prompt.ask("Enterprise? (y/n)", default="n").lower() == "y"
            show_store_packages_menu(ctx["subscription_type"], is_enterprise)
